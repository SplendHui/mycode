...
void doSort(int myid, int local_n, int np)
{
	...
	for (i = 0; i < np; i++)
	{
		...
		if (partner != MPI_PROC_NULL)
		{
			...
			MPI_Send(mykeys, local_n, MPI_INT, partner, 0, MPI_COMM_WORLD);
			MPI_Recv(receive, local_n, MPI_INT, partner, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
